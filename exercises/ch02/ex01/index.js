/**
 * 　このNodeプログラムは標準入力からテキストを読みこみ。
 * 　テキスト中の文字の頻度を計算し、降順でヒストグラムを
 *   表示する。実行するにはNode12以降が必要。
 *
 * 　UNIX系の環境であれば、次のように実行する　：
 * 　　　node charfreq.js < corpus.txt
 */

// このクラスでは、Mapが拡張して、キーがマップ上に存在しないときに、
// getメソッドがnullの代わりに指定した値を返すようにする。
...
class Histogram {
  ...
  add(text) {
    // テキストとから空白文字を取り除き、すべての文字を大文字に変換する
  ...
  toString() {
    ...
    // 文字数順にソードする。文字数が同じ場合は、アルファベット順でソートする。
    entries.sort((a, b) => {
      // ソート順を定義する関数
      if (a[1] === b[1]) {
        // 文字数が同じ場合は、
